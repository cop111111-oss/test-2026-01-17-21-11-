...
const App: React.FC = () => {
  ...
  const SectionHeader = ({ title, subtitle }: { title: string; subtitle?: string }) => (
    <div className="mb-20 text-center">
      <h2 className="text-4xl md:text-6xl font-black mb-6 tracking-tighter uppercase text-white">{title}</h2>
          ...
          <div className="max-w-4xl">
            ...
            <h1 className="text-6xl md:text-8xl font-black mb-10 leading-[0.9] tracking-tighter">
              {content.heroTitle.split(' ').map((word, i) => (
                <span key={i} className={i > 2 ? 'text-transparent bg-clip-text bg-gradient-to-br from-purple-400 to-purple-800' : 'text-white'}>
                  {word}{' '}
                </span>
              ))}
            </h1>
      ...
      <section className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {[
            ...
            <div key={i} className="group p-10 bg-white/[0.03] backdrop-blur-2xl border border-white/5 rounded-[2.5rem] hover:border-purple-500/30 transition-all duration-500">
              <div className="w-16 h-16 bg-purple-900/20 rounded-2xl flex items-center justify-center text-purple-500 mb-8 group-hover:scale-110 group-hover:bg-purple-600 group-hover:text-white transition-all duration-500">
                {stat.icon}
              </div>
            ...
            {content.services.map((service, i) => (
              <div key={service.id} className="group cursor-pointer">
                <div className="relative overflow-hidden rounded-[2rem] mb-8 aspect-[4/5] border border-white/5">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent group-hover:via-black/0 transition-all duration-500"></div>
                  <div className="absolute bottom-8 left-8 right-8">
                     <span className="text-[10px] font-black tracking-widest uppercase text-purple-400 mb-2 block opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">Solution 0{i+1}</span>
                ...
                <div className="h-[1px] w-full bg-white/5 mb-6 group-hover:bg-purple-500/30 transition-all duration-500"></div>
                <button onClick={() => setCurrentPage('services')} className="flex items-center space-x-2 text-white/40 group-hover:text-purple-500 font-black text-xs uppercase tracking-widest transition-all duration-300 ml-2">
    ...
    <div className="max-w-7xl mx-auto px-4 py-32 space-y-32 bg-transparent">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
        <div className="relative">
          ...
          <div className="relative p-10 bg-white/[0.03] backdrop-blur-xl rounded-[2.5rem] border border-purple-500/20 shadow-2xl">
        ...
        <div className="relative group">
           <div className="absolute inset-0 bg-purple-600/10 blur-[100px] opacity-0 group-hover:opacity-100 transition-opacity duration-1000"></div>
        ...
        {content.services.map((service, index) => (
          <div key={service.id} className={`flex flex-col ${index % 2 === 1 ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-24 items-center group`}>
            <div className="flex-1">
              <span className="text-purple-600/10 font-black text-[10rem] leading-none block mb-[-2rem] select-none">0{index + 1}</span>
              <h3 className="text-5xl font-black mb-8 tracking-tighter text-white group-hover:text-purple-400 transition-colors duration-300">{service.title}</h3>
              ...
              <button onClick={() => setCurrentPage('contact')} className="px-12 py-5 bg-white text-black hover:bg-purple-600 hover:text-white transition-all duration-500 rounded-full font-black uppercase tracking-widest text-xs">
                Inquiry Now
              </button>
            </div>
    ...
    <div className="max-w-7xl mx-auto px-4 py-32 bg-transparent">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {content.portfolio.map((item) => (
          <div key={item.id} className="group bg-white/[0.02] backdrop-blur-2xl border border-white/5 rounded-[2.5rem] overflow-hidden hover:border-purple-500/40 hover:translate-y-[-10px] transition-all duration-500">
            <div className="h-72 overflow-hidden relative">
              <img src={item.imageUrl} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110 grayscale group-hover:grayscale-0" alt={item.title} />
              <div className="absolute top-6 left-6 bg-black/80 backdrop-blur-md text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest border border-white/10">{item.category}</div>
            </div>
            <div className="p-10">
              <h3 className="text-2xl font-black mb-4 text-white group-hover:text-purple-400 transition-colors">{item.title}</h3>
    ...
    <div className="max-w-7xl mx-auto px-4 py-32 bg-transparent">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-16 mt-20">
        <div className="lg:col-span-2 space-y-12">
          <div className="p-10 bg-white/[0.03] backdrop-blur-xl rounded-[2.5rem] border border-white/5 relative overflow-hidden group">
            ...
            <form className="space-y-8" onSubmit={handleFormSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <input name="name" type="text" className="w-full bg-black/40 border border-white/10 rounded-xl p-5 focus:border-green-500/50 outline-none transition-all placeholder-zinc-700 text-white" placeholder="홍길동" required disabled={isSubmitting} />
                </div>
                <div className="space-y-3">
                  <label className="text-[13px] font-bold text-zinc-400 ml-1">연락처</label>
                  <input name="phone" type="tel" className="w-full bg-black/40 border border-white/10 rounded-xl p-5 focus:border-green-500/50 outline-none transition-all placeholder-zinc-700 text-white" placeholder="010-0000-0000" required disabled={isSubmitting} />
                </div>
              </div>
              <div className="space-y-3">
                <label className="text-[13px] font-bold text-zinc-400 ml-1">이메일</label>
                <input name="email" type="email" className="w-full bg-black/40 border border-white/10 rounded-xl p-5 focus:border-green-500/50 outline-none transition-all placeholder-zinc-700 text-white" placeholder="example@email.com" required disabled={isSubmitting} />
              </div>
              <div className="space-y-3">
                <label className="text-[13px] font-bold text-zinc-400 ml-1">프로젝트 내용</label>
                <textarea name="message" className="w-full bg-black/40 border border-white/10 rounded-xl p-5 h-40 focus:border-green-500/50 outline-none transition-all resize-none placeholder-zinc-700 text-white" placeholder="어떤 프로젝트를 구상 중이신가요?" required disabled={isSubmitting}></textarea>
              </div>
              <button type="submit" disabled={isSubmitting} className={`w-full py-6 rounded-2xl font-bold transition-all text-white flex items-center justify-center space-x-3 text-lg ${isSuccess ? 'bg-[#1fb155] shadow-[0_10px_30px_rgba(31,177,85,0.4)]' : 'bg-[#1fb155] hover:bg-[#19a349] active:scale-[0.98]'} disabled:opacity-70 disabled:cursor-not-allowed`}>
                {isSubmitting ? (
